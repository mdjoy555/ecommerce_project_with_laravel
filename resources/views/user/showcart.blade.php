        ...
        @foreach($cart as $carts)
          <tr style="background-color: black;">
            ...
            <td style="padding: 10px; color: white;">
              ...
              {{$carts->price}}</td>
            <td style="padding: 10px; color: white;">
              <a href="{{url('deletecart',$carts->id)}}" class="btn btn-danger"
              onclick="return confirm('Are you sure you want to delelte?')">Delete</a>
            </td>
          </tr>
        @endforeach
      @else
        <tr><td><td><td style="padding: 10px; font-size: 20px;">
        There is no cart</td></td></td></tr>
      @endif
      </table>
      <button class="btn btn-success mt-2">Confirm Order</button>
    </form>
    </div>
